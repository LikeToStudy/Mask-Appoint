$(function () {
    ...
    // 判断是否登录变量
    var falgOfAddressAndPhoneNumber = false;
     ...
     *
     */
    var stockJSON = JSON.stringify({ "productName" : "N95防护口罩" });
    $.ajax({
        type: "post",
    ...
    // 判断是否填写地址和收件号码
    function checkAddressAndPhoneNumber() {
        $.ajax({
            type: "post",
            url: "http://localhost:8080/isOffAddrInfo",
            // 关闭异步请求，修改全局变量
            async: false,
            dataType: "json", //返回数据类型
            contentType : 'application/json',
            success: function(data){
                 ...
                 *    data = { "result" : result }
                 *
                 *    result    已经填写了相应的地址和收件号码
                 *
                 */
                if(data.result){
                    falgOfAddressAndPhoneNumber = true;
                }
            }
        });
    }
    ...
    function appointment () {
        checkAddressAndPhoneNumber();
        if(falgOfAddressAndPhoneNumber){
            var data = { "quantity" : $("#quantity").val() , "productName" : "N95防护口罩"};
